...
class OllamaMCP:
    ...
    async def ollama_chat(self, messages: list[dict[str, str]]) -> Any:
        ...
        """
        conversation = [{"role":"assistant", "content": f"你尽量使用工具。你可以使用以下函数：{[ tool.name for tool in self.tools]}"}]
...
async def major(question):
    ...
    messages=[
        {
            "role": "system",
            "content": "你是一个听话的助手，上下文中有一系列工具。"
                    "你的任务是满足我的各种要求。不要自己生成函数结果。"
                    "简洁地回复一条简短消息，提及调用函数，但不提供函数输出本身。"
                    "将该简短消息放在'response'属性中。调用函数时的operation请从上下文的工具参数中选取,不要自己翻译出参数。"
                    "例如：'好的,我会运行magicoutput函数并返回输出。'同时用正确的参数填充'tool'属性。"
        }
   ]
    messages+=question
...
def ask_agent(question):
    ...
    return asyncio.run(major(question))
...
if __name__ == "__main__":
    ...
    #     [{
    #         "role": "user", 
    #         "content": "请使用文件处理工具统计文件 E:/workspace/OllamaMCP/testtext.txt 的行数"
    #     }]
    # ))
    ask_agent("请使用文件处理工具统计文件 E:/workspace/OllamaMCP/testtext.txt 的行数")